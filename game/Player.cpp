...
// RAVEN END
	...
	declManager->FindType( DECL_ENTITYDEF, "damage_apathy", false, false );
...
void idPlayer::UpdateHudStats( idUserInterface *_hud ) {
	...
	temp = _hud->State().GetInt( "player_apathy", "-1");
	if( temp != apathy ) {
		_hud->SetStateInt   ( "player_apathyDelta", temp == -1 ? 0 : (temp - apathy) );
		_hud->SetStateInt	( "player_apathy", apathy < -100 ? -100 : apathy );
		_hud->SetStateFloat	( "player_apathypct", idMath::ClampFloat ( 0.0f, 1.0f, (float)apathy / 1000.0f ) );
		_hud->HandleNamedEvent ( "updateApathy" );
	}
	// Boss bar
...
void idPlayer::UpdateApathy( void )
{
	apathy++;
	if(apathy > 2500)
	{
		apathy = 2400;
		const idDict *damageDef = gameLocal.FindEntityDefDict( "damage_softfall", false );
				Damage( NULL, NULL, vec3_origin, "damage_softfall", 0.1f, 0 );
	}
	if ( hud ) {
		hud->SetStateInt( "player_apathy", apathy );
	}
	gameLocal.Printf("Apathy: %d\n", apathy);
}
/*
==============
idPlayer::Think
...
void idPlayer::DamageFeedback( idEntity *victim, idEntity *inflictor, int &damage ) {
	...
	bool armorHit = false;
	//Hurting others makes the apathy go away
	apathy -= damage * 10;
	if(apathy < 0)
		apathy = 0;
